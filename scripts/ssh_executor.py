...
class DemoExecutor(RemoteExecutor):
    ...
    def _get_demo_output(self, command: str) -> str:
        """명령어에 맞는 데모 출력 반환"""
        if 'df -h' in command: return self.demo_data['disk_usage']
        elif 'free -m' in command: return self.demo_data['memory_usage']
        elif 'top -bn1' in command: return self.demo_data['cpu_usage']
        elif 'uptime' in command: return self.demo_data['uptime']
        elif 'ps aux' in command and 'Z' in command: return self.demo_data['zombie']
        elif '/proc/loadavg' in command: return self.demo_data['load']
        elif 'swap' in command.lower(): return self.demo_data['swap']
        elif 'file-nr' in command: return self.demo_data['open_files']
        elif 'ss -t' in command: return self.demo_data['network_conn']
        elif 'uname -r' in command: return self.demo_data['kernel']
        elif 'kubectl get nodes' in command: return self.demo_data['k8s_nodes']
        elif 'kubectl version' in command: return self.demo_data['k8s_version']
        else: return "OK"
