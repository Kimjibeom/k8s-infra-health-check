...
```
k8s-infra-health-check/
│
├── 📄 k8s-infra-check.sh        # 메인 실행 스크립트 (진입점)
├── 📄 README.md                 # 프로젝트 문서
│
├── 📁 config/                   # 설정 파일
...
```bash
cd /path/to/k8s-infra-health-check
```
...
export K8S_INVENTORY_PATH="/path/to/k8s-infra-health-check/config/inventory.yaml"
```
...
```bash
./k8s-infra-check.sh --type monthly
```
...
```bash
# 개발 Bastion에서
./k8s-infra-check.sh --cluster dev_cluster

# 스테이징 Bastion에서
./k8s-infra-check.sh --cluster stg_cluster

# 운영 Bastion에서
./k8s-infra-check.sh --cluster prd_cluster
```

**출력 디렉토리 지정**

```bash
./k8s-infra-check.sh -o /var/reports/k8s-check
```

**최소 출력 + JSON만**

```bash
./k8s-infra-check.sh -q --json
```
...
# 주간: k8s_cluster_check_YYYY_Wnn.csv, k8s_cluster_check_YYYY_Wnn.docx
# 월간: k8s_cluster_check_YYYY_MM.csv, k8s_cluster_check_YYYY_MM.docx
```
...
| 순서 | 작업 | 명령어/확인 |
|------|------|-------------|
| 1 | 프로젝트 디렉터리로 이동 | `cd /path/to/k8s-infra-health-check` |
| 2 | 실행 권한 + 의존성 | `chmod +x k8s-infra-check.sh` / `pip3 install pyyaml python-docx` |
| 3 | 인벤토리 설정 | `config/inventory.yaml` 편집 |
| 4 | (선택) 환경변수 | `export SSH_USER=...` 등 |
| 5 | 점검 실행 | `./k8s-infra-check.sh` 또는 `--cluster` / `--env` 조합 |
| 6 | 결과 확인 | `ls output/` |
...
스크립트(`k8s-infra-check.sh`)는 아래 옵션을 지원하며, `-c`/`--cluster`·`-t`·`-e`·`-o` 등은 내부적으로 Python 스크립트로 전달됩니다.
...
1. **실행 위치**  
   ...
   - **각 클러스터만 점검할 때는 해당 클러스터 Bastion에 접속한 뒤**, 그 Bastion에서 `./k8s-infra-check.sh --cluster <해당_클러스터>` 로 실행하는 구성을 권장합니다. (같은 대역에서 직접 SSH 접속이 되는 환경 전제.)
...
3. **보고서**  
   - CSV/DOCX는 기본적으로 `output/` 아래에 `k8s_cluster_check_YYYY_Wnn.csv`(주간) 또는 `k8s_cluster_check_YYYY_MM.docx`(월간) 형식으로 생성됩니다.  
   - `--output-dir`로 경로를 바꿀 수 있습니다.  
   - 이미 생성된 CSV만 있을 때 DOCX가 필요하면: `python3 scripts/csv_to_docx.py [CSV경로]` (경로 생략 시 `output/` 내 최신 CSV 사용).

4. **환경변수**  
   - `SSH_USER`, `SSH_PRIVATE_KEY_PATH`, `K8S_INVENTORY_PATH` 등으로 SSH·인벤토리 경로를 오버라이드할 수 있습니다. (보안 섹션 참고.)
...
export K8S_INVENTORY_PATH="/secure/config/inventory.yaml"
...
```bash
0 9 * * 1 /path/to/k8s-infra-health-check/k8s-infra-check.sh >> /var/log/k8s-check.log 2>&1
...
```bash
0 9 1 * * /path/to/k8s-infra-health-check/k8s-infra-check.sh --type monthly >> /var/log/k8s-check-monthly.log 2>&1
...
```bash
0 9 * * 1 SSH_USER=admin SSH_PRIVATE_KEY_PATH=/home/admin/.ssh/id_rsa /path/to/k8s-infra-check.sh >> /var/log/k8s-check.log 2>&1
...
0 9 * * 1 /path/to/k8s-infra-check.sh --cluster dev_cluster >> /var/log/k8s-check-dev.log 2>&1
0 9 * * 1 /path/to/k8s-infra-check.sh --cluster prd_cluster >> /var/log/k8s-check-prd.log 2>&1
```
## 📊 출력 예시
...
📝 보고서 생성 중...
✅ 보고서 생성 완료:
   - CSV: ./output/k8s_cluster_check_2025_W49.csv
   - DOCX: ./output/k8s_cluster_check_2025_W49.docx
```
